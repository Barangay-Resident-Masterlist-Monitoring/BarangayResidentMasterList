...
const dashboard = () => {
  ...
  const secretaryUsers = users.filter(u => u.role === 'Secretary');

  const countBy = (field, ageBuckets = false) => {
    const counts = {};
    secretaryUsers.forEach(u => {
      let value = u[field];
      ...
      <div className="container flex-grow-1 py-5">
        ...
        <section className="mb-5 mt-4">
          <h3 className="text-center mb-4 fw-semibold" style={{ color: '#222' }}>
            <FaInfoCircle className="me-2" /> System Usage Guide for Secretaries
          </h3>
              ...
              <tbody>
                {[
                  ...
                  },
                  {
                    ...
                    desc: 'Hover or tap chart segments to view values. Charts update with latest data.',
                  },
                  {
                    icon: <FaListAlt color="#4A90E2" size={20} />,
                    title: 'Types of Charts',
                    desc: 'Pie chart for sex, bar for age & occupation, doughnut for civil status.',
                  },
                  {
                    icon: <FaCompass color="#4A90E2" size={20} />,
                    title: 'Navigation Tips',
                    desc: 'Use sidebar or top menu to switch sections like profile, chart, users, etc.',
                  },
                  {
                    icon: <FaLock color="#4A90E2" size={20} />,
                    title: 'Data Privacy & Accuracy',
                    desc: 'Secretaries handle sensitive data—ensure correctness before submission.',
                  },
                  {
                    icon: <FaInfoCircle color="#4A90E2" size={20} />,
                    title: 'Important Note',
                    desc: '⚠️ Secretaries can update records. Use caution and authorization for changes.',
                  },
                ].map((row, i) => (
                  <tr
                    key={i}
